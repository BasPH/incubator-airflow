...
class DataSet:
    ...
    def __init__(self, qualified_name=None, data=None, **kwargs):
        ...
        self._data.update(dict((key, value) for key, value in kwargs.items() if key in set(self.attributes)))

        if data:
            if "qualifiedName" in data:
                self._qualified_name = data.pop("qualifiedName")

            self._data = dict((key, value) for key, value in data.items() if key in set(self.attributes))
