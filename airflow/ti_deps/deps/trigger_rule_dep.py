...
class TriggerRuleDep(BaseTIDep):
    ...
    def _get_dep_statuses(self, ti, session, dep_context=None):
        ...
        if ti.task.trigger_rule == TriggerRule.DUMMY:
        ...
        successes, skipped, failed, upstream_failed, done = self._get_states_count_upstream_ti(
            ti=ti, finished_tasks=dep_context.finished_tasks, session=session
        )
    ...
    def _evaluate_trigger_rule(  # pylint: disable=too-many-branches
        ...
        if flag_upstream_failed:
            if task.trigger_rule == TriggerRule.ALL_SUCCESS:
                ...
                if successes or skipped:
                    ti.set_state(State.SKIPPED, session)
            elif task.trigger_ruler == TriggerRule.ONE_SUCCESS:
        ...
        if task.trigger_rule == TriggerRule.ONE_SUCCESS:
            if successes <= 0:
                yield self._failing_status(
                    ...
                    .format(task.trigger_rule, upstream_tasks_state, task.upstream_task_ids))
        elif task.trigger_rule == TriggerRule.ONE_FAILED:
            if not failed and not upstream_failed:
                yield self._failing_status(
                    ...
                    "upstream_tasks_state={1}, upstream_task_ids={2}"
                    .format(task.trigger_rule, upstream_tasks_state, task.upstream_task_ids))
        elif task.trigger_rule == TriggerRule.ALL_SUCCESS:
            ...
            if num_failures > 0:
                yield self._failing_status(
                    ...
                    .format(task.trigger_rule, num_failures, upstream_tasks_state,
            ...
            if num_successes > 0:
                yield self._failing_status(
                    ...
                    "upstream_tasks_state={2}, upstream_task_ids={3}"
                    .format(task.trigger_rule, num_successes, upstream_tasks_state,
                            task.upstream_task_ids))
            ...
            if not upstream_done:
                yield self._failing_status(
                    ...
                    "upstream_task_ids={3}"
                    .format(task.trigger_rule, upstream_done, upstream_tasks_state,
                            task.upstream_task_ids))
        elif task.trigger_rule == TriggerRule.NONE_FAILED:
            ...
            if not upstream_done or (skipped > 0):
                yield self._failing_status(
                    ...
                    "upstream_tasks_state={2}, upstream_task_ids={3}"
                    .format(task.trigger_rule, skipped, upstream_tasks_state,
                            task.upstream_task_ids))
        else:
            yield self._failing_status(
                reason="No strategy to evaluate trigger rule '{0}'.".format(task.trigger_rule))
